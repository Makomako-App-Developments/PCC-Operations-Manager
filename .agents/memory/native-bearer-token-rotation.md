...
Completion retries that receive the permanent conflict “Job must be claimed and in progress before completion” are stale when the server already holds the terminal job state. Offer a narrowly scoped local discard action for only those failed completion records; never delete or roll back the server job.