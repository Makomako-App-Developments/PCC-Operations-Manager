...
**How to apply:** Treat upload, reopened mobile rendering, saved-photo deletion, and desktop rendering as one end-to-end contract. Verify all four whenever the Storm Patrol photo response shape or URL handling changes. Desktop images from protected `/api/uploads/*` routes must be fetched through the authenticated API client and displayed with temporary object URLs; a plain `<img src>` cannot send the bearer token.